...
function processRound(roomCode) {
    ...
    if (activePlayersCount === 2) {
        // ========== 分支 A: 兩人對決模式 ==========
        const [playerA, playerB] = choicesData;
        const choiceA = playerA.choice;
        const choiceB = playerB.choice;

        // 【規則三: 終極對決】優先判定
        if ((choiceA === 0 && choiceB === 100) || (choiceA === 100 && choiceB === 0)) {
            winnerId = choiceA === 100 ? playerA.playerId : playerB.playerId;
        } else {
            // 如果不是終極對決，則進行最基礎的勝負計算，忽略所有其他追加規則
            targetNumber = (choiceA + choiceB) * 0.8 / 2;
            const diffA = Math.abs(choiceA - targetNumber);
            const diffB = Math.abs(choiceB - targetNumber);
            ...
            if (diffA < diffB) {
                ...
                winnerId = null; // 平手則無勝利者
            }
        }
    } else {
        // ========== 分支 B: 三人或以上模式 (沿用上一版已修復的邏輯) ==========
        ...
        if (activePlayersCount > 2) {
            ...
            if (gambitPlayers.length === 1) {
                ...
                gambitPlayers.forEach(p => disqualifiedPlayerIds.add(p.playerId));
            }
        }
        if (activePlayersCount <= 4) {
            ...
            choicesData.forEach(p => { if (choiceCounts[p.choice] > 1) { disqualifiedPlayerIds.add(p.playerId); } });
        }
        ...
        if (winnerCandidates.length > 0) {
            ...
            if (activePlayersCount <= 3 && winnerId !== null) {
                if (choicesData.find(c => c.playerId === winnerId).choice === Math.round(targetNumber)) { penalty = 2; }
            }
        } else {
            const sum = choicesData.reduce((acc, curr) => acc + (curr.choice > -1 ? curr.choice : 0), 0);
    ...
    activePlayers.forEach(player => {
        ...
        else if (player.id === winnerId) { /* No change */ }
